...
The system SHALL evaluate a LangGraph thread for context compression after each completed user/AI turn and before the chat SSE stream emits `done`.

#### Scenario: Completed turn exceeds threshold

- **WHEN** a chat stream finishes graph execution without a pending approval and the estimated effective short-term context reaches the configured compression threshold
- **THEN** the backend MUST run context compression before emitting the final `done` SSE event

#### Scenario: Completed turn is below threshold

- **WHEN** a chat stream finishes graph execution and the estimated effective short-term context is below the configured compression threshold
- **THEN** the backend MUST skip compression and finish the stream without emitting a context compression progress step
...
The system SHALL report context compression status through the existing `progress` SSE event contract.

#### Scenario: Compression starts

- **WHEN** backend compression begins for a chat run
- **THEN** the backend MUST emit a `progress` event containing a `context_compression` step with `running` status and a user-readable detail

#### Scenario: Compression succeeds

- **WHEN** backend compression updates the thread state successfully
- **THEN** the backend MUST emit a `progress` event containing the `context_compression` step with `success` status

#### Scenario: Compression fails

- **WHEN** backend compression fails after it started
- **THEN** the backend MUST emit a `progress` event containing the `context_compression` step with `failed` status and MUST continue to emit the stream `done` event
