...
## **一、Agent 知识底座规划**
当前项目根目录说明文档很完整，但更像“人读的项目介绍”。建议升级为 Agent-first knowledge base：

```text
docs/
├─ ARCHITECTURE.md          # 系统分层、核心模块、依赖方向
├─ AGENT_RUNTIME.md         # LangGraph 节点、状态、审批、中断恢复
├─ SSE_PROTOCOL.md          # metadata/token/progress/artifact/error/done 等事件契约
├─ ARTIFACT_CONTRACT.md     # artifact 类型、状态机、版本链、存储规则
├─ TOOL_REGISTRY.md         # skill/tool 权限、输入输出、失败语义
├─ RAG_CONTRACT.md          # 知识库文件、切片、检索、引用规则
├─ SECURITY.md              # 文件、workspace、OnlyOffice、模型调用安全边界
├─ RELIABILITY.md           # 超时、重试、降级、恢复策略
├─ EVALS.md                 # Agent 评测集、指标、回归门禁
└─ PRODUCT_RUBRIC.md        # 教学设计质量、课件质量、教案质量评分标准
```

根目录 `AGENTS.md` 保持精简，只做导航和高优先级约束。详细规则按模块渐进加载。这一点和 OpenAI 的“AGENTS.md 作为目录，不作为百科全书”的建议一致。

## **二、Agent 工具与 Skill Harness**
...
## **七、多 Agent 分工规划**
当前系统已有主 Agent、附件分析 Agent、artifact Agent。下一步可以明确角色：

- Router Agent：只负责意图路由
- Planner Agent：只负责教学设计规划
- Retrieval Agent：只负责资料检索和引用整理
- Artifact Agent：负责 PPT/DOCX/HTML 生成
- Revision Agent：负责差量修改
- Critic Agent：负责教学质量审核
- Safety Agent：负责输入输出风险判断
- Eval Agent：负责自动评分和回归判断

关键原则：不要让一个 Agent 同时负责“生成、审核、执行、判定成功”。生成和验证要分开。
